...
var UrlDisplaySettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Use alias").setDesc("By default, the website title from URL metadata is used as the display text. If enabled, the text in brackets ([]) will be used.").addToggle(
      (value) => value.setValue(this.plugin.settings.useAlias).onChange((value2) => {
        this.plugin.settings.useAlias = value2;
        this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Show favicon").setDesc("If disabled, the pane will not show the favicon, but only the text.").addToggle(
      (value) => value.setValue(this.plugin.settings.showFavicon).onChange((value2) => {
        this.plugin.settings.showFavicon = value2;
        this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Cache mode").setDesc("Choose cache mode for saving favicons. This will only take effect when show favicon is set to true.").addDropdown((value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Notice mode").setDesc("You can customize the type of notifications when URL parsing finishes.").addDropdown((value) => {
...
var EXTERNAL_LINK = /(\[([^\[\]]+?)?\]\()?https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_\+.~#?&\/='" ]*)\)?/g;
var EXCLUDE = [".avif", ".apng", ".jpg", ".jpeg", ".png", ".gif", ".svg", ".ico", ".bmp", ".webp", ".mp4", ".avi", ".mkv", ".mov", ".flv", ".webm", ".awebp?"];
  ...
  constructor(leaf, plugin, processor) {
    ...
    this.onHeaderMenu = (menu) => {
      menu.addItem((item) => {
        item.setTitle("Refresh list").setIcon("refresh-cw").onClick(async () => {
          const currentMarkdownView = this.app.workspace.getActiveViewOfType(import_obsidian2.MarkdownView);
    ...
    this.updateDisplay = () => {
      ...
      if (!this.processor.isExtracting && !this.processor.activeNotehaveUrl) {
        container.createEl("p", { text: "No valid URL found \u{1F604}" });
      }
    ...
    this.handleMousedown = (event) => {
      ...
      if (event.button === 0) {
        ...
        (_a = this.processor.activeMarkdownView) == null ? void 0 : _a.setEphemeralState({ line });
...
var markdownProcessor = class {
  constructor(plugin) {
    this.process = (0, import_obsidian4.debounce)(async (markdownView) => {
      var _a, _b;
      this.initState();
      ...
      if (this.activeMarkdownView) {
        ...
        if (!activeNoteUrl) {
          ...
          if (currentMarkdownView) {
            if (((_a = currentMarkdownView.file) == null ? void 0 : _a.path) === ((_b = markdownView == null ? void 0 : markdownView.file) == null ? void 0 : _b.path)) {
              this.updateView();
            }
          } else {
            this.updateView();
    ...
    this.parseUrl = async (markdownView) => {
      this.isParsing = true;
      this.updateView();
...
var UrlDisplayPlugin = class extends import_obsidian5.Plugin {
  ...
  registerCommand() {
    ...
    this.addCommand({
      ...
      name: "Refresh list",
      checkCallback: (checking) => {
        const currentMarkdownView = this.app.workspace.getActiveViewOfType(import_obsidian5.MarkdownView);
  ...
  async activateView() {
    if (this.app.workspace.getActiveViewOfType(import_obsidian5.MarkdownView)) {
      ...
      new import_obsidian5.Notice("Move focus into markdown \u{1F609}");
    }
  }
  onunload() {
